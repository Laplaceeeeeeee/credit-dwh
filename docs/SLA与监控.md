...
| 4 | **跨链路对账** ⭐ | 两条链路都"成功"，数字一样吗 | 双引擎：计数 0、比率 ≤ 5e-5 ｜ 批流：笔数 0、金额 ≤ 0.01% | 引擎语义差异、分区漏挂、重复消费 | ✅ 双引擎已实现；⚠️ 批流未实现（第 19 步） |
...
### 5.3 批流对账（⚠️ 第 19 步未实施，规则先定义）

| 检查 | 规则 | 容忍度 | 状态 |
|---|---|---|---|
| 离线 vs 实时日粒度笔数 | `count(offline) == count(realtime)` | 严格相等 | ⚠️ 未实施 |
| 离线 vs 实时日粒度金额 | `abs(offline - realtime) / offline` | ≤ 0.01% | ⚠️ 未实施 |

> **诚实说明**：本项目**没有真实的实时数据源**，第 19 步（实时链路）是可砍项且尚未开工，
> 所以这两条规则**只是定义，没有实现、没有跑过**。
> 简历上**不能写"做过批流对账"** —— 只能写"设计了批流对账的判据"。
> 这也正是把规则写下来的意义：**接口先定清楚，链路上线时对账才有落脚点**
> （实时表 `ads_loan_daily_realtime` 已在 `docs/lineage.yaml` 里以
> `status: planned` 标出，图中画成虚线）。
