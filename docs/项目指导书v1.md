...
> **配套文档**：`v1.2_Project.md`（回答"做什么、为什么"）｜ 本文档回答"**具体怎么一步步做**"
> 生成日期：2026-09-12 ｜ **实测修订：2026-09-15**
> 适用对象：SQL 会窗口函数、Python 能看懂、数仓零基础
> 技术栈：Windows 11 + Docker + Python 3.11 + MySQL 8（端口 **3307**）+ Git
> 预计总投入：**阶段二约 100 小时**
> 数据实况（**本地实测**）：`accepted_2007_to_2018Q4.csv.gz`，374.4 MB，
> **物理 2,260,702 行 = 表头 1 + 页脚 33 + 真实数据 2,260,668**，保留 **137** 列

---

## ⚠️⚠️ 必读：2026-09-15 实测修订（6 处，其中 3 处会直接卡死流程）

本文档初版的部分代码在**真实环境跑不通**。以下是逐条实测修正，
**请以本节为准**，正文对应处已同步更新（标有 🔧）。

| # | 问题 | 后果 | 修正 |
|---|---|---|---|
| **1** | 🔴 **`skipfooter` 与 `chunksize`/`nrows` 互斥** | 6 个代码块**一行都跑不起来**，直接抛 `ValueError` | 新增 **`src/prepare_raw.py` 预处理步骤**，只剔一次页脚，之后全读 Parquet |
| **2** | 🔴 **页脚有 33 行且散布全文**，不是 1 行 | 原设计只跳最后 1 行，另 32 行混进 `id` 列 | 全文件扫描剔除；真实数据行 **2,260,668**（不是 2,260,700） |
| **3** | 🔴 **`member_id` 整列为空**（+ 共 14 列全空） | "客户维 + 一人多贷"设计不成立 | 改 `dim_applicant` 申请人画像维；剔除 14 列 |
| **4** | 🟠 MySQL 保留字 `year_month` | DDL 报 `ERROR 1064` | **所有列名加反引号** |
| **5** | 🟠 端口不一致（compose 3307 / config 3306） | 连不上库 | `config.DB['port'] = 3307` |
| **6** | 🟠 Windows 控制台 GBK 编码 | `UnicodeEncodeError`，emoji 一 print 就崩 | `utils.py` 导入时自动 `reconfigure(utf-8)` |

**另有 5 个非阻塞但必须知道的坑**（详见 **附录 C 排错手册**）：

7. **分块读取 dtype 漂移** —— 同一列在不同块被推断成不同类型，写 Parquet 报 schema 不匹配
8. **`id` 存成 float64** —— 读出来变 `68407277.0`；业务主键必须存字符串
9. **MySQL 排序规则冲突** —— `to_sql` 建的表与 DDL 建的表 collation 不同，JOIN 时 `ERROR 1267`
10. **`Int64` 的 NA 在 mask 时不覆盖** —— 会把"从未还款"的坏账误判成 CURRENT
11. **PowerShell 脚本中文乱码** —— UTF-8 无 BOM 的 `.ps1` 被按 GBK 读，破坏引号导致 `ParserError`

> 📌 **完整的实测数据、验证过程与结论见 `配置进度与问题记录.md`。**
> 项目已按修正后的方案**端到端跑通**，可直接参照 `src/` 下的实际代码。
...
| 你的状态 | 从哪开始 |
|---|---|
| 完全没开始 | **第 0.5 步（新增的预处理）** → 顺序往下 |
| 卡在某个报错 | 直接查 **附录 C 排错手册（已扩充到 22 条）** |
| 想知道某步做到什么程度算完成 | 每步末尾的 **✅ 验收标准** |
| 想快速看全貌 | 先读 **§0.3 全景流程图** |

> 🔑 **新增的第 0.5 步（`prepare_raw`）不可跳过。**
> 它替代了原设计中"每次读取都 skipfooter"的错误做法 ——
> 那个做法在本机 pandas 上根本执行不了。
...
def read_clean(usecols=None, nrows=None) -> pd.DataFrame:
    """
    读取**已剔除页脚**的干净数据（Parquet）。

    ⚠️ 🔧 实测修正：不要在这里用 skipfooter！
       pandas 的 skipfooter 与 chunksize / nrows **互斥**，会抛：
           ValueError: 'skipfooter' not supported for iteration
           ValueError: 'skipfooter' not supported with 'nrows'
       而且这个文件的页脚有 **33 行且散布全文**，skipfooter 也处理不了。

       正确做法：先用 src/prepare_raw.py 剔除一次，产出干净 Parquet，
       之后所有脚本读这里。既没有页脚问题，也用上了 C 引擎（快 20 倍）。
    """
    return pd.read_parquet(clean_source(), columns=usecols)
...
- [ ] `docker compose ps` 中 mysql 容器是 `Up` 状态
- [ ] MySQL 能连上（**端口 3307**），`SHOW DATABASES` 有 `credit_dwh`
- [ ] `git status --short` 中**没有** `data/raw/` 下任何文件
- [ ] `src\__init__.py` 已创建
- [ ] 在项目根目录执行 `python -c "from src import utils; print('ok')"` 无报错
- [ ] GitHub 仓库已有第一次提交并且推送成功
- [ ] **`pyarrow` 已安装**（`pip install pyarrow`）—— 预处理产物用 Parquet

---

## 🆕 第 0.5 步：预处理——剔除页脚（约 2 小时，**不可跳过**）

> ⚠️ **这一步是 2026-09-15 实测后新增的，它替代了原设计中的错误做法。**

### 0.5.1 为什么必须单独做这一步

原设计让每个脚本在读取时都用 `skipfooter=1` 跳页脚。**实测发现两个致命问题**：
...
pd.read_csv(p, nrows=5, skipfooter=1, engine="python")
# ValueError: 'skipfooter' not supported with 'nrows'
```

`skipfooter` 只能用于**一次性全量读取**。而全量读只能用 python 引擎，
226 万行要 **20–40 分钟**。也就是说：原设计下所有分块脚本**一行都跑不起来**。
...
这个文件**不是一份单纯导出**，而是**多个年度分段拼接**而成，段间夹着章节标题与汇总行。
`skipfooter=1` 只能删掉**最后一行**，另外 **32 行**会混进 `id` 列，
并让 pandas 把 `id` 推断成字符串，进而引发 Parquet schema 不匹配。

### 0.5.2 决定：只剔除一次，产出干净数据
...
```
文件总行数 2,260,702 − 1(表头) − 33(页脚) = 应为 2,260,668
实际产出 2,260,668 行  ✅ 一致
保留列数 137；耗时 82.6s
```
...
| 文件 | 内容 |
|---|---|
| `data/clean/loans_full.parquet` | 全量 2,260,668 行 × 137 列（337 MB，snappy） |
| `data/clean/loans_sample.parquet` | 子集 200,000 行（调流程用，27 MB） |
| `data/clean/footer_line.txt` | 被剔除的 33 行页脚原文（**留证**） |
| `data/clean/clean_meta.json` | 行数、列数、页脚行号等元信息 |

### 0.5.3 这一步顺带解决的四个问题

| 问题 | 处理 |
|---|---|
| 页脚污染 | 全文件扫描，按"`id` 首字段必须是纯数字"剔除 33 行 |
| 14 个整列为空的字段 | 整体剔除（`member_id` + 7 个 `sec_app_*` 等），151 → 137 列 |
| **分块读取 dtype 漂移** | 用首块探测 schema，**固定 dtype** 后传给每一次读取 |
| **`id` 被存成 float64** | 主键强制为 **string**（数值型主键有精度隐患，且读数会带 `.0`） |

### 0.5.4 性能对照（为什么这个方案更好）

| 方案 | 引擎 | 全量读取耗时 | 能否分块 |
|---|---|---|---|
| ❌ 每次 `skipfooter` | python 引擎 | **20–40 分钟** | ❌ 不能 |
| ✅ **一次预处理 + 读 Parquet** | C 引擎 | **80 秒** | ✅ 能 |

> 🔑 **面试价值**：这是一个典型的"**用一次性预处理换取整体性能与正确性**"的工程决策。
> 讲清楚"为什么不在每次读取时处理、为什么要留证页脚原文"，
> 比单纯说"我清洗了数据"有说服力得多。

### ✅ 第 0.5 步验收标准

- [ ] `data/clean/loans_full.parquet` 已生成，行数 = **2,260,668**
- [ ] `data/clean/footer_line.txt` 有 **33 行**（页脚留证）
- [ ] `clean_meta.json` 中 `footer_lines_skipped = 33`、`clean_rows_full = 2260668`
- [ ] 校验 `id` 是 **string** 类型且全为纯数字：

  ```powershell
  & .\.venv\Scripts\python.exe -c @"
  import pandas as pd
  df = pd.read_parquet(r'data\clean\loans_sample.parquet', columns=['id'])
  print(df['id'].dtype, df['id'].head(3).tolist())
  print('全纯数字:', bool(df['id'].str.fullmatch(r'\d+').all()))
  "@
  # 期望：string ['68407277', ...] 全纯数字: True
  ```

- [ ] `git commit -m "feat: 新增预处理步骤，剔除33行散布页脚"`
...
def profile():
    ...
    reader = read_clean_chunks(usecols=None, chunksize=CHUNK)
    for chunk in reader:
        n_chunks += 1
        n_rows += len(chunk)
...
def main():
    ...
    print(f"→ 预期 DWD 行数 = ODS 行数 - REJECT 行数 = 2,260,668 - {int(no_issue.sum())}"
...
| 压缩包大小 | 374.4 MB |
| 原始行数（物理，含表头与页脚） | 2,260,702 |
| **有效数据行数** | **2,260,668** |
| 字段数 | 151（剔除 14 个整列为空的字段后 **保留 137**） |
...
**实测结果**：本文件的页脚**不是 1 行，而是 33 行**，而且**散布在文件中部**，
不是只在末尾。原因是文件由多个年度分段拼接，段间夹着章节标题与汇总行。
**处理方式**：由 `src/prepare_raw.py` 全文件扫描剔除（详见**第 0.5 步**）。
**行数结论**：物理 2,260,702 − 表头 1 − 页脚 33 = **真实数据 2,260,668**。

### 2.2 🚨 `member_id` 列整列为空（废列）
**实测结果：2,260,668 行中 `member_id` 非空值为 0 —— 该列 100% 为空字符串。**
...
**行数守恒登记**：ODS 2,260,668 = DWD **2,260,668**，**差异 0 行**。
（页脚 33 行已在预处理阶段剔除，DWD 无 REJECT 记录。）
...
- 范围：**Apr-2008 ~ Sep-2018** 期间**已批准并放款**的 2,260,668 笔贷款
- 无：拒绝样本、全量申请样本
...
### 边界 0.5：32 行记录缺少放款日期
- **实测**：**0 行**存在 `issue_d` 缺失。
  （早期版本曾以为有 32 行缺日期，实测证明那 32 行**就是散布的页脚行**，
  并非真实记录缺字段。已在预处理阶段剔除，因此 DWD 的 REJECT 计数为 0。）
- **影响**：无法归属放款批次，不能参与 Vintage / 趋势 / 月末快照
- **处理**：页脚行在预处理阶段已剔除；DWD 的 REJECT 计数为 **0**，
  **行数守恒登记为 ODS 2,260,668 = DWD 2,260,668（差 0 行）**
...
- [ ] 加权平均利率写明了"为什么加权"
- [ ] **十项数据边界全部写明**（含 `member_id` 整列为空、33 行散布页脚、DPD 为规则推导），且每项都有"处理方式"
...
def load_ods(full_reload: bool = True):
    ...
    # ⭐ ODS 层就做行数守恒校验
    # 2,260,668 = 文件总行数 2,260,702（物理行数） − 1 行页脚；若为 2,260,702（物理行数） 说明 skipfooter 没生效
    assert after == 2_260_700, (
        f"ODS 行数异常：{after}，期望 2,260,668。"
        f"若为 2,260,702（物理行数） 说明页脚行未被 skipfooter 跳过"
    )
    log.info("✅ 行数守恒校验通过（ODS = 2,260,668）")
...
def main(full_reload: bool = True):
    ...
    # ⭐ 行数守恒硬校验：预期 2,260,668 − 32 = 2,260,668
...
def build():
    ...
    reader = pd.read_csv(
        ...
        usecols=PERF_COLS, # 🔧 读干净 Parquet（已剔页脚）
    )
    parts = []
...
- [ ] `dwd_loan_fact` 行数 = **2,260,668**（= ODS 行数，REJECT 为 0）
...
基于 LendingClub 公开信贷数据（**2,260,668 笔贷款 / 137 字段**），构建 ODS-DWD-DWS-ADS
...
| # | 现象 | 原因 | 解决 |
|---|---|---|---|
| **C-1** | ODS 行数 = 2,260,702 或更多，且含 `Total amount funded...` | **页脚有 33 行且散布全文**，不是只在末尾 | 🔧 **不要用 `skipfooter`**（它与 chunksize 互斥，且处理不了中部页脚）。跑 `python -m src.prepare_raw` 全文件剔除 |
| **C-2** | `id` 列出现 `Total amount funded...` 或 NaN | 页脚行混入 | 同上：跑 `prepare_raw`。⚠️ 不要与 C-18 混淆：`member_id` **整列全空**是**数据本身**的缺陷，不是脏行，也不要去"修复"它 —— 应从设计中剔除该列，客户维降级为申请人画像维 |
| **C-3** | `int_rate` 报 "could not convert string" | 原始值带 `%` | `str.replace('%','')` 后再 `to_numeric` |
| **C-4** | ODS 写入 MySQL 超过 3 小时 | 逐行写入慢 | 🔧 **实测：200,000 行约 86 秒**，推算 226 万行约 **16 分钟**（远快于原估计的 1–3 小时）。只入关键 39 列，`method="multi"` + 分批 2 万行 |
| **C-5** | `skipfooter` 相关 `ValueError` | **`skipfooter` 与 `chunksize`/`nrows` 互斥**（pandas 2.2.2 与 3.0.5 均如此） | 🔧 **本项目已不再使用 `skipfooter`**。改走 `prepare_raw` + Parquet。见 C-23 |
...
| **C-18** | `member_id` 整列是 NULL / 客户维建不出来 | **该列在本地文件中整列为空字符串**（实测 2,260,668 行全空） | 从设计中剔除该列；`dim_customer` 改为 `dim_applicant`（主键 = `loan_id`）。**绝不要**用 `annual_inc+addr_state` 之类拼"伪客户 ID" |
...
| **C-23** | `ValueError: 'skipfooter' not supported for iteration` / `... with 'nrows'` | **`skipfooter` 与 `chunksize`/`nrows` 互斥**，只能用于一次性全量读 | 🔧 走 `src/prepare_raw.py` 预处理一次，之后读干净 Parquet。**这是本项目最重要的架构决策** |
| **C-24** | `ValueError: Table schema does not match schema used to create file` | **分块读取 dtype 漂移**：同一列在不同块被 pandas 推断成不同类型（实测 `verification_status_joint` 在块1是 string、块2是 double） | 用首块探测 schema，**固定 dtype** 后传给每一次 `read_csv`。`prepare_raw.py` 已实现 |
| **C-25** | `id` 读出来是 `68407277.0`，或主键校验"不是纯数字"全挂 | 主键被存成了 **float64** | 业务主键**强制为 string**：`prepare_raw.py` 的 `STRING_KEYS = {"id", "zip_code"}`。数值型主键还有精度隐患 |
| **C-26** | `(1267, "Illegal mix of collations (utf8mb4_0900_ai_ci,IMPLICIT) and (utf8mb4_unicode_ci,IMPLICIT) for operation '='")` | **排序规则冲突**：DDL 建表用了 `utf8mb4_unicode_ci`，而 pandas `to_sql` 建表跟随 MySQL 8 库默认 `utf8mb4_0900_ai_ci`。两套规则不同的列 JOIN 时 MySQL 直接拒绝 | 🔧 **全库统一用 `utf8mb4_0900_ai_ci`**：DDL 里显式声明，`dim_build.unify_collation()` 兜底。不要用 `SET NAMES ... COLLATE` 硬改，那样会和既有表打架 |
| **C-27** | PowerShell 脚本报 `ParserError: The string is missing the terminator` | **`.ps1` 文件是 UTF-8 无 BOM，被 Windows PowerShell 5.1 按 GBK 读**，中文变乱码并截断字符串 | ① 脚本内**只用 ASCII**；② 或写成 **UTF-8 with BOM**。本项目 `06_ops/shell/run_pipeline.ps1` 采用前者 |
| **C-28** | Python 脚本 `UnicodeEncodeError: 'gbk' codec can't encode character '\u2705'` | 中文 Windows 控制台默认 GBK，脚本里的 ✅❌⚠️ 一 print 就崩 | `src/utils.py` 导入时调用 `setup_console_encoding()` 自动 `reconfigure(encoding="utf-8")`。**内联 `python -c` 脚本不受保护**，需先 `$env:PYTHONIOENCODING='utf-8'` |
| **C-29** | MySQL 报 `ERROR 1064 ... near 'year_month CHAR(7)'` | **`year_month` 是 MySQL 保留字**（`YEAR_MONTH` 是 interval 单位） | 🔧 **DDL 里所有列名统一加反引号**，从根上免疫保留字问题 |
| **C-30** | 连接被拒绝 / 连不上 MySQL | compose 映射的是 **3307**，而 `config.py` 默认写的是 3306 | 改 `config.DB['port']` 为 **3307**，或设环境变量 `DB_PORT=3307` |
